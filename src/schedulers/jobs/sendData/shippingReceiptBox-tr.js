...
async function extractAssortment(_id, sp) {
    try {
        ...
        (async () => {
            await ShippingReceiptBoxRespond.findByIdAndUpdate(_id, { status: ShippingReceiptBoxRespond.Status.I });
...
module.exports = {
    insert2AX: async function (_id) {
        try {
            const sp = await ShippingReceiptBoxRespond.findById(_id);
            var allBarcodes = await extractAssortment(_id, sp);
            ...
            if (result.recordset.length > 0) {
                ...
                table.columns.add('RECID', sql.BigInt, { nullable: false });
                table.columns.add('CREATEDBY', sql.VarChar(50), { nullable: false });
                table.rows.add(sp.documentNum, sp.date, 'Zammler', 0, 0, 'ua1', (Number(recId) + i), 'erdis');
                await pool.request().bulk(table);
            }
            await ShippingReceiptBoxRespond.findByIdAndUpdate(_id, { status: ShippingReceiptBoxRespond.Status.C, sentToAX: true });
